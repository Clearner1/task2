...
当前“预处理”实现以媒体探测和状态落库为主，尚未扩展到完整的转码、波形图和视频封面生成流水线。这一部分已作为后续增强项记录。
...
当前版本已经完成题目要求中的核心架构、交互闭环和 JSON 导出，但仍有两类增强项应继续推进：

1. 将“24 小时无人值守运行”从当前的持久化与恢复语义，进一步扩展到真正的后台巡检、失败重试重调度和运行统计面板。
2. 将“预处理”从媒体探测升级到完整的标准化流水线，例如转码、波形图和视频封面生成。

这两项后续工作会作为独立 issue 管理，不在本次 `P0` 文档补齐中直接修改实现。
